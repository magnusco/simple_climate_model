...
if __name__ == "__main__":
    ...
    mu_1 = p.f_A * p.C_C * p.r_LC * p.a_LW
    
    a_0 = p.sigma * p.e_E * (1 - lambda_0)
    b_0 = - p.sigma * p.e_E * mu_0
    c_0 = p.alpha
    d_0 = - p.beta
    e_0 = gamma_0 * p.P_s
    ...
    equilib_temp = scipy.optimize.fsolve(F, np.array([280, 280]), xtol = 1e-9)
    ...
    R_E_out = p.sigma * p.e_E * equilib_temp[0]**4
    R_A_out = p.sigma * p.e_A * equilib_temp[1]**4
    P_lat = (p.alpha + p.beta) * (equilib_temp[0] - equilib_temp[1])

    print("Short-wave in: ", P_E_in, P_A_in)
    print("Long-wave in: ", R_E_in, R_A_in)
    print("Long-wave out: ", R_E_out, R_A_out)
    print("Total latent: ", P_lat)
